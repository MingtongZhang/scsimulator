...
public class Environment extends Thread {
	...
	public class Updater extends Thread{
		public void leftmove(Occupier o) {
			//System.out.println("left tried");
			boolean cango = true;
			if((o.position-o.speed)-o.extrasize<0){
				cango=false;
				//System.out.println("left failed 1");
			}
			for(Agent a : agents) {
				if(a!=o) {
...
//				System.out.println("Updater running");
				for(Agent a : agents) {
					...
					if(a.movement.equals("right")){
						rightmove(a);	}
//					System.out.println(a.name+"'s position:"+a.position);					
				}
				//write 1d world
				for(Agent a : agents) {
				...
				for(Agent a : agents) {
					for(int n=1;n<10;n++) {
						if((a.position+a.extrasize)+1>=0&&(a.position-a.extrasize)-1>=0&&(a.position-a.extrasize)-1<=Constant.WL-1&&(a.position+a.extrasize)+1<=Constant.WL-1)
						{if(line[(a.position+a.extrasize)+1].occuppied||line[(a.position-a.extrasize)-1].occuppied) {
							a.collision=true;break;
						}}
				}
					}
				for(Agent a: agents) {
	...
	public class RequestProcessor extends Thread {
		...
		public void run() {
			while(true) {
				//System.out.println("for processor"+agents.get(0).position);
				//System.out.println("RequestProcessor running");
				Request therequest;
				try {					
					therequest = rp.take();
					...
					//System.out.println(therequest.from.name+rp.size());
				} catch (InterruptedException e) {
					// TODO Auto-generated catch block
					e.printStackTrace();
				}
